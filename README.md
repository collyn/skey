...
- 🚀 **Nhẹ:** một file `.so` duy nhất, không daemon, không cần `/dev/uinput`
...
| **Output Mode** | Surrounding Text / Preedit | Chế độ hiển thị text đang gõ |
...
| **Server riêng** | Có (daemon process) | Không |
| **Giao tiếp** | IPC (pipe/socket) | Direct function call |
| **Quyền `/dev/uinput`** | Cần | Không cần |
| **Systemd service** | Cần | Không cần |
| **Auto-restore** | autoNonVnRestore + ddFreeStyle | autoNonVnRestore + ddFreeStyle |
| **Deferred commit** | Có | Có (80ms timer) |
| **Cài đặt** | Phức tạp (server + service) | `make install` + `skey-setup` |
