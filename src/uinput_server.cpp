...
passwd *lookupUser(const std::string &name, std::vector<char> &buf, passwd &pwd) {
...
public:
    ...
    bool init() {
        ...
        const int keys[] = {KEY_BACKSPACE, KEY_LEFTCTRL, KEY_LEFTSHIFT,
                            KEY_U, KEY_ENTER, KEY_0, KEY_1, KEY_2, KEY_3,
    ...
    while (running.load()) {
        ...
        if (FD_ISSET(server.get(), &readfds)) {
            ...
            if (newClient) {
                ...
                bool ok = getsockopt(newClient.get(), SOL_SOCKET, SO_PEERCRED,
                                     &cred, &credLen) == 0 &&
        ...
        if (client && FD_ISSET(client.get(), &readfds)) {
            ...
            if (n == static_cast<ssize_t>(sizeof(int32_t))) {
                ...
                size_t available = static_cast<size_t>(n) - sizeof(count) - sizeof(textLen);
