...
static std::pair<std::string, std::string> feedKeys(skey::VietnameseEngine &eng,
...
int main(int argc, char **argv) {
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    runTest(
        {cat, "tieengs → tiếng", skey::InputMethod::Telex, "tieengs", "tiếng"});
    ...
    // More complex words
    runTest(
        {cat, "dduwowcj → được", skey::InputMethod::Telex, "dduwowcj", "được"});
    runTest(
        {cat, "khoong → không", skey::InputMethod::Telex, "khoong", "không"});
    runTest({cat, "nguwowif → người", skey::InputMethod::Telex, "nguwowif",
             "người"});
    runTest(
        {cat, "nhuwngx → những", skey::InputMethod::Telex, "nhuwngx", "những"});
    runTest({cat, "truwowngf → trường", skey::InputMethod::Telex, "truwowngf",
             "trường"});
    runTest({cat, "dduwowngf → đường", skey::InputMethod::Telex, "dduwowngf",
             "đường"});
    runTest({cat, "thuwowngf → thường", skey::InputMethod::Telex, "thuwowngf",
             "thường"});
    runTest(
        {cat, "thoong → thông", skey::InputMethod::Telex, "thoong", "thông"});
    ...
    runTest({cat, "nguwowix → ngưỡi (free mark)", skey::InputMethod::Telex,
             "nguwowix", "ngưỡi",
    ...
    runTest({cat, "cuar → của", skey::InputMethod::Telex, "cuar", "của"});

    // Tricky words
    runTest({cat, "nghieeng → nghiêng", skey::InputMethod::Telex, "nghieeng",
             "nghiêng"});
    runTest({cat, "ngoawn → ngoăn (aw→ă fixed)", skey::InputMethod::Telex,
             "ngoawn", "ngoăn", "aw→ă at non-start position, now works"});
    runTest(
        {cat, "khuyur → khuỷu", skey::InputMethod::Telex, "khuyur", "khuỷu"});
    runTest({cat, "nghieepj → nghiệp", skey::InputMethod::Telex, "nghieepj",
             "nghiệp"});

    // Everyday phrases
    runTest({cat, "tooi → tôi", skey::InputMethod::Telex, "tooi", "tôi"});
    runTest({cat, "laf → là", skey::InputMethod::Telex, "laf", "là"});
    runTest({cat, "cos → có", skey::InputMethod::Telex, "cos", "có"});
    runTest({cat, "thoix → thời", skey::InputMethod::Telex, "thowif", "thời"});
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    runTest({cat, "nhaan → nhân", skey::InputMethod::Telex, "nhaan", "nhân"});
    runTest({cat, "cuowcs → cước", skey::InputMethod::Telex, "cuowcs", "cước"});
    runTest({cat, "cows → cỡ", skey::InputMethod::Telex, "cows", "cớ",
             "bamboo: ow+s = ơ+s = ớ"});
    runTest({cat, "xuans → xuán", skey::InputMethod::Telex, "xuans", "xuán",
             "xu + a + n + s = xuán (sắc on a). For xuân use xuaan"});
    runTest({cat, "xuaan → xuân", skey::InputMethod::Telex, "xuaan", "xuân"});
    runTest({cat, "hocj → học", skey::InputMethod::Telex, "hocj", "học"});
    runTest({cat, "vieecj → việc", skey::InputMethod::Telex, "vieecj", "việc"});
    runTest({cat, "nhas → nhá", skey::InputMethod::Telex, "nhas", "nhá"});
    runTest({cat, "ngay → ngay", skey::InputMethod::Telex, "ngay", "ngay"});
    runTest({cat, "vuis → vui", skey::InputMethod::Telex, "vui", "vui",
             "tone s on 'i' at end: vu + i + s → vúi? vui is vui, no tone "
             "transforms"});

    // đ with tone
    runTest({cat, "ddas → đá", skey::InputMethod::Telex, "ddas", "đá"});
    runTest({cat, "ddi → đi", skey::InputMethod::Telex, "ddi", "đi"});
    runTest({cat, "ddeens → đến", skey::InputMethod::Telex, "ddeens", "đến"});
    runTest({cat, "ddau → đau", skey::InputMethod::Telex, "ddau", "đau",
             "đ + a + u = đau. For đâu use ddaau"});
    runTest({cat, "ddaau → đâu", skey::InputMethod::Telex, "ddaau", "đâu"});

    // Words with initial ng/ngh
    runTest({cat, "ngux → ngũ", skey::InputMethod::Telex, "ngux", "ngũ"});
    runTest({cat, "nghef → nghè", skey::InputMethod::Telex, "nghef", "nghè",
             "ngh + e + f"});
    runTest({cat, "nghir → nghỉ", skey::InputMethod::Telex, "nghir", "nghỉ"});

    // Words with gi
    runTest({cat, "gif → gì", skey::InputMethod::Telex, "gif", "gì"});
    runTest({cat, "giowf → giờ", skey::InputMethod::Telex, "giowf", "giờ"});
    runTest({cat, "gioir → giỏi", skey::InputMethod::Telex, "gioir", "giỏi",
             "gi + o + i + r = giỏi"});
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    runTest({cat, "ooo → oo (undo ô)", skey::InputMethod::Telex, "ooo", "oo"});
    // Toggle once, then one-shot: dd→đ, ddd→dd, dddd→ddd.
    runTest(
        {cat, "ddd → dd (toggle once)", skey::InputMethod::Telex, "ddd", "dd"});
    // Same for circumflex vowels: aa→â, aaa→aa, aaaa→aaa.
    runTest(
        {cat, "aaa → aa (toggle once)", skey::InputMethod::Telex, "aaa", "aa"});
    runTest(
        {cat, "eee → ee (toggle once)", skey::InputMethod::Telex, "eee", "ee"});
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    runTest({cat, "resstore → restore (P5 undo + engine restore)",
             ...
             "ss cancels é (P5), engine auto-restore fixes the rest", false,
             false, true});
    ...
    runTest({cat, "ressponse → response (P5 undo, tone kept)",
             ...
             "P4 bypass removed — P5 handles double tone keys", false, false,
             true});
    ...
    // Simulate: type "restore" (with undo bypass), reset, then type "tôi"
    {
      ...
      if (pass) {
        ...
        ++gFailed;
        std::cout << RED << "  FAIL" << RESET << "  bypass reset: word1=\""
                  << word1 << "\" word2=\"" << word2 << "\"\n";
      }
    }
    ...
    // internally, so englishBypass_ is no longer set by undo paths.
    {
      ...
      if (pass) {
        ...
        std::cout << RED << "  FAIL" << RESET
                  << "  bypass flag: on=" << bypassOn << " off=" << bypassOff
                  << "\n";
      }
    }
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    // Commit + reset + new word
    {
      ...
      if (pass) {
        ...
        ++gFailed;
        std::cout << RED << "  FAIL" << RESET << " reset + new word: first=\""
                  << first << "\" second=\"" << second << "\"\n";
      }
    }
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    runBackspaceTest(
        {cat, "vaix BS → vai", skey::InputMethod::Telex, "vaix", 1, "vai"});
    runBackspaceTest({cat, "tieengs BS → tiêng", skey::InputMethod::Telex,
                      "tieengs", 1, "tiêng"});
    runBackspaceTest({cat, "dduwowcj BS → đươc", skey::InputMethod::Telex,
                      "dduwowcj", 1, "đươc"});
    runBackspaceTest({cat, "tieengs BSx2 → tiên", skey::InputMethod::Telex,
                      "tieengs", 2, "tiên"});
    runBackspaceTest(
        {cat, "dd BS → d", skey::InputMethod::Telex, "dd", 1, "d"});
    runBackspaceTest(
        {cat, "oo BS → o", skey::InputMethod::Telex, "oo", 1, "o"});
    runBackspaceTest({cat, "aws BS → ă (remove tone)", skey::InputMethod::Telex,
                      "aws", 1, "ă"});
    runBackspaceTest(
        {cat, "tieeng BS → tiên", skey::InputMethod::Telex, "tieeng", 1, "tiên",
         "backspace removes last raw char 'g', recompose tieen → tiên"});
    runBackspaceTest(
        {cat, "tie BS → ti", skey::InputMethod::Telex, "tie", 1, "ti"});
    // Backspace all the way
    runBackspaceTest(
        {cat, "xin BSx3 → empty", skey::InputMethod::Telex, "xin", 3, ""});
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    runTest({cat, "hello → hello (no tone keys)", skey::InputMethod::Telex,
             "hello", "hello"});
    runTest({cat, "world → world (real-time restore)", skey::InputMethod::Telex,
             "world", "world", "r=hỏi on o → invalid → real-time restore",
             false, false, true});
    ...
    runTest({cat, "wood → wood (real-time restore)", skey::InputMethod::Telex,
             "wood", "wood", "wood→wôd → invalid → real-time restore", false,
             false, true});
    ...
    runTest({cat, "ook → ook (real-time restore)", skey::InputMethod::Telex,
             "ook", "ook", "ook→ôk invalid → real-time restore", false, false,
             true});
    runTest({cat, "vaai → vaai (real-time restore)", skey::InputMethod::Telex,
             "vaai", "vaai", "vaai→vâi invalid → real-time restore", false,
             false, true});
    runTest({cat, "aloo → aloo (real-time restore)", skey::InputMethod::Telex,
             "aloo", "aloo", "aloo→alô invalid → real-time restore", false,
             false, true});
    runTest({cat, "baa → bâ (valid syllable, keep)", skey::InputMethod::Telex,
             "baa", "bâ", "bâ is valid Vietnamese → is_valid=true → keep"});
    runTest({cat, "mee → mê (valid syllable, keep)", skey::InputMethod::Telex,
             "mee", "mê", "mê is valid Vietnamese → is_valid=true → keep"});
    runTest({cat, "caw → că (valid syllable, keep)", skey::InputMethod::Telex,
             "caw", "că", "că is valid Vietnamese → is_valid=true → keep"});
    runTest({cat, "mow → mơ (valid syllable, keep)", skey::InputMethod::Telex,
             "mow", "mơ", "mơ is valid Vietnamese → is_valid=true → keep"});

    // Uppercase D: NDD → NĐ
    runTest({cat, "NDD → NĐ (uppercase D)", skey::InputMethod::Telex, "NDD",
             "NĐ", "uppercase DD after N → Đ"});

    // Mixed case: NDd → NĐ
    runTest({cat, "NDd → NĐ (mixed case Dd)", skey::InputMethod::Telex, "NDd",
             "NĐ", "mixed Dd after N → Đ"});

    // Uppercase: toggle once then one-shot — DDD → DD
    runTest({cat, "DDD → DD (toggle once)", skey::InputMethod::Telex, "DDD",
             "DD", "dd→đ once, then one-shot: DD stays"});

    runTest({cat, "NDDD → NDD (toggle once)", skey::InputMethod::Telex, "NDDD",
             "NDD", "dd→đ once, then one-shot: DD stays"});

    // "đẹp" → typed as dd + e + e + p? No...
    // dd + e + j + p = đẹ? Actually:
    // ddepj → đ + ê + p + j... hmm
    // dd + ej + p = đ + ẹ + p = đẹp
    runTest({cat, "ddepj → đẹp", skey::InputMethod::Telex, "ddepj", "đẹp"});

    // Numbers mixed with letters
    runTest({cat, "abc123 → abc123 (ignored)", skey::InputMethod::Telex,
             "abc123", "abc",
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    runTest({cat, "truwocs → trước (real word kept)", skey::InputMethod::Telex,
             "truwocs", "trước", "trước is in the dictionary", false, false,
             true, true});
    runTest({cat, "khoong → không (real word kept)", skey::InputMethod::Telex,
             "khoong", "không", "không is in the dictionary", false, false,
             true, true});

    // dd marker still protects abbreviations in dictionary mode.
    runTest({cat, "ddc → đc (dd marker protected)", skey::InputMethod::Telex,
             "ddc", "đc", "dd signals intentional đ — auto-restore skips",
    ...
    runTest({cat, "luoojcw → lược (w on uô)", skey::InputMethod::Telex,
             "luoojcw", "lược", "w converts ô→ơ (uô→ươ), matches unikey", false,
             false, true, true});
    ...
    runTest({cat, "luowj → lượ (tone intermediate kept)",
             ...
             "lượ base is a strict prefix of lược's base", false, false, true,
             true});
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    runTest({cat, "AS → Á", skey::InputMethod::Telex, "As", "Á"});
    runTest({cat, "AW → Ă", skey::InputMethod::Telex, "Aw", "Ă"});
    runTest({cat, "AWS → Ắ", skey::InputMethod::Telex, "Aws", "Ắ"});
    runTest({cat, "DD → Đ", skey::InputMethod::Telex, "Dd", "Đ"});
    runTest({cat, "Vieetj → Việt", skey::InputMethod::Telex, "Vieetj", "Việt"});
    runTest({cat, "EE → Ê", skey::InputMethod::Telex, "Ee", "Ê"});
    runTest({cat, "OO → Ô", skey::InputMethod::Telex, "Oo", "Ô"});
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    // Non-letter keys should be ignored by VietnameseEngine
    {
      ...
      if (pass) {
        ++gPassed;
        std::cout << GREEN << "  PASS" << RESET << "  digit ignored in Telex\n";
      } else {
        ++gFailed;
        std::cout << RED << "  FAIL" << RESET << "  digit should be ignored\n";
      }
    }
    {
      skey::VietnameseEngine eng;
      ...
      if (pass) {
        ++gPassed;
        std::cout << GREEN << "  PASS" << RESET << "  punctuation ignored\n";
      } else {
        ++gFailed;
        std::cout << RED << "  FAIL" << RESET
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    runTest(
        {cat, "tie6ng1 → tiếng", skey::InputMethod::VNI, "tie6ng1", "tiếng"});
    runTest({cat, "Vie6t5 → Việt", skey::InputMethod::VNI, "Vie6t5", "Việt"});
    runTest(
        {cat, "d9u7o7c5 → được", skey::InputMethod::VNI, "d9u7o7c5", "được"});
    runTest({cat, "kho6ng → không", skey::InputMethod::VNI, "kho6ng", "không"});
    runTest(
        {cat, "ngu7o7i2 → người", skey::InputMethod::VNI, "ngu7o7i2", "người"});
    runTest(
        {cat, "nhu7ng4 → những", skey::InputMethod::VNI, "nhu7ng4", "những"});
    runTest({cat, "tru7o7ng2 → trường", skey::InputMethod::VNI, "tru7o7ng2",
             "trường"});
    runTest({cat, "d9u7o7ng2 → đường", skey::InputMethod::VNI, "d9u7o7ng2",
             "đường"});
    runTest({cat, "thu7o7ng2 → thường", skey::InputMethod::VNI, "thu7o7ng2",
             "thường"});
    runTest({cat, "xin → xin", skey::InputMethod::VNI, "xin", "xin"});
    runTest({cat, "cha2o → chào", skey::InputMethod::VNI, "cha2o", "chào"});
    runTest({cat, "to6i → tôi", skey::InputMethod::VNI, "to6i", "tôi"});
    runTest({cat, "ca1c → các", skey::InputMethod::VNI, "ca1c", "các"});
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    runBackspaceTest({cat, "d9 BS → d", skey::InputMethod::VNI, "d9", 1, "d"});
    runBackspaceTest(
        {cat, "a61 BSx2 → a", skey::InputMethod::VNI, "a61", 2, "a"});
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    runTest({cat, "w → ư (bare w = ư)", skey::InputMethod::Telex, "w", "ư",
             nullptr, true});
    runTest(
        {cat, "ws → ứ", skey::InputMethod::Telex, "ws", "ứ", nullptr, true});
    runTest(
        {cat, "wf → ừ", skey::InputMethod::Telex, "wf", "ừ", nullptr, true});
    runTest(
        {cat, "ow → ơ", skey::InputMethod::Telex, "ow", "ơ", nullptr, true});
    runTest(
        {cat, "ows → ớ", skey::InputMethod::Telex, "ows", "ớ", nullptr, true});
    runTest(
        {cat, "aw → ă", skey::InputMethod::Telex, "aw", "ă", nullptr, true});
    runTest(
        {cat, "dd → đ", skey::InputMethod::Telex, "dd", "đ", nullptr, true});
    runTest(
        {cat, "aa → â", skey::InputMethod::Telex, "aa", "â", nullptr, true});
    runTest(
        {cat, "ee → ê", skey::InputMethod::Telex, "ee", "ê", nullptr, true});
    runTest(
        {cat, "oo → ô", skey::InputMethod::Telex, "oo", "ô", nullptr, true});

    runTest({cat, "tieengs → tiếng", skey::InputMethod::Telex, "tieengs",
             "tiếng", nullptr, true});
    runTest({cat, "dduwowcj → được", skey::InputMethod::Telex, "dduwowcj",
             "được", nullptr, true});
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    auto bracket = [](const char *name, const char *keys,
                      const char *expected) {
      return TestCase{"Telex+BracketUO / Basic",
                      name,
    ...
    runTest({cat, "] → ư (with ShortW too)", skey::InputMethod::Telex, "]", "ư",
             nullptr, true, true});
    runTest({cat, "[ → ơ (with ShortW too)", skey::InputMethod::Telex, "[", "ơ",
             nullptr, true, true});
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    // Switch from Telex to VNI mid-composition (reset then use VNI)
    {
      ...
      if (pass) {
        ...
        ++gFailed;
        std::cout << RED << "  FAIL" << RESET << "  Telex→VNI: actual=\""
                  << actual << "\"\n";
      }
    }
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    runTest({cat, "hoaf → hòa (traditional, FreeMarking off)",
             skey::InputMethod::Telex, "hoaf", "hòa"});
    runTest({cat, "banj → bạn", skey::InputMethod::Telex, "banj", "bạn"});
    runTest({cat, "toans → toán", skey::InputMethod::Telex, "toans", "toán"});
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    // Simulate typing "xin chào" word by word with reset between
    {
      ...
      if (pass) {
        ...
        ++gFailed;
        std::cout << RED << "  FAIL" << RESET << "  sequential: w1=\"" << w1
                  << "\" w2=\"" << w2 << "\"\n";
      }
    }
    ...
    // "Việt Nam"
    {
      ...
      if (pass) {
        ++gPassed;
        std::cout << GREEN << "  PASS" << RESET << "  Việt → reset → Nam\n";
      } else {
        ++gFailed;
        std::cout << RED << "  FAIL" << RESET << "  Việt Nam: w1=\"" << w1
                  << "\" w2=\"" << w2 << "\"\n";
      }
    }
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    // Single character
    runTest({cat, "single 'a'", skey::InputMethod::Telex, "a", "a"});
    ...
    // Tone-only input (starts with tone key → ignored)
    {
      ...
      if (pass) {
        ...
        ++gFailed;
        std::cout << RED << "  FAIL" << RESET << "  tone key 's' alone: got '"
                  << eng.getComposed() << "'\n";
      }
    }

    // Double transform over word boundary
    runTest({cat, "hoa → hoa (no transform, not hòa)", skey::InputMethod::Telex,
             "hoa", "hoa"});

    // Triple letters
    runTest({cat, "buoorn → buồn", skey::InputMethod::Telex, "buoofn", "buồn",
             "b + u + oo + f + n = buồn"});
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    auto runRapidWords = [](const RapidTestCase &rtc) {
      ...
      for (size_t w = 0; w < rtc.wordKeys.size(); ++w) {
        ...
        if (result != exp) {
          std::cout << RED << "  FAIL" << RESET << "  " << rtc.name << " word "
                    << w << ": \"" << rtc.wordKeys[w] << "\" → \"" << result
                    << "\" (expected \"" << exp << "\")\n";
          return;
        }
        eng.reset();
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    runCrossMethod({"tiếng", "tieengs", "tie6ng1", "tieengs", "tiếng"}, true,
                   true);
    runCrossMethod({"việt", "vieetj", "vie6t5", "vieetj", "việt"}, true, true);
    runCrossMethod({"được", "dduwowcj", "d9u7o7c5", "dduwowcj", "được"}, true,
                   true);
    runCrossMethod({"không", "khoong", "kho6ng", "khoong", "không"}, true,
                   true);
    runCrossMethod({"người", "nguwowif", "ngu7o7i2", "nguwowif", "người"}, true,
                   true);
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    // Stress: create/destroy many engines
    {
      ...
      if (pass) {
        ++gPassed;
        std::cout << GREEN << "  PASS" << RESET
                  << "  100 engine create/destroy cycles\n";
      } else {
        ++gFailed;
        std::cout << RED << "  FAIL" << RESET
    ...
    // Stress: long composition then reset
    {
      ...
      for (int i = 0; i < 50; ++i) {
        ...
        if (c + p != "tiếng") {
          ++gFailed;
          std::cout << RED << "  FAIL" << RESET
                    << "  stress long comp iteration " << i << "\n";
          goto stress_done;
        }
      }
      ++gPassed;
      std::cout << GREEN << "  PASS" << RESET
                << "  50 composition cycles (reset→tieengs→verify)\n";
    stress_done:
      (void)0;
    }
    ...
    // Stress: switch methods rapidly
    {
      ...
      if (pass) {
        ++gPassed;
        std::cout << GREEN << "  PASS" << RESET
                  << "  50 rapid method switch cycles\n";
      } else {
        ++gFailed;
        std::cout << RED << "  FAIL" << RESET
                  << "  rapid method switch cycles\n";
      }
    }
